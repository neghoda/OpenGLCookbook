...
SceneOit::SceneOit() : angle(0.0f), tPrev(0.0f), rotSpeed(glm::pi<float>() / 8.0f), sphere(1.0f, 40, 40) {}
...
void SceneOit::compileAndLinkShader() {
	try {
		prog.compileShader("Shaders/oit.vert");
		prog.compileShader("Shaders/oit.frag");
		prog.link();
		prog.use();
	}
