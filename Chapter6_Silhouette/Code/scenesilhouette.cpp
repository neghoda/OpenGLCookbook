...
void SceneSilhouette::compileAndLinkShader() {
    try {
        prog.compileShader("Shaders/silhouette.vert", GLSLShader::VERTEX);
        prog.compileShader("Shaders/silhouette.frag", GLSLShader::FRAGMENT);
        prog.compileShader("Shaders/silhouette.geom", GLSLShader::GEOMETRY);
        prog.link();
        prog.use();
    }
