...
SceneRefractCube::SceneRefractCube() : angle(0.0f), tPrev(0.0f), rotSpeed(glm::pi<float>() / 8.0f), teapot(14, glm::mat4(1.0f)) {}
...
void SceneRefractCube::compileAndLinkShader() {
    try {
        prog.compileShader("Shaders/cubemap_refract.vert");
        prog.compileShader("Shaders/cubemap_refract.frag");
        prog.link();
        prog.use();
    }
