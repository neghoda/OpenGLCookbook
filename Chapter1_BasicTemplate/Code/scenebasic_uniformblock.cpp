...
void SceneBasic_UniformBlock::compile()
{
	try {
#ifdef __APPLE__
		prog.compileShader("Shaders/basic_uniformblock_41.vert");
		prog.compileShader("Shaders/basic_uniformblock_41.frag");
#else
		prog.compileShader("Shaders/basic_uniformblock.vert");
		prog.compileShader("Shaders/basic_uniformblock.frag");
#endif
		prog.link();
		prog.use();
